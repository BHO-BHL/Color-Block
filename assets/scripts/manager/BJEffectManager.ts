/*
 * @Author: carolsail 
 * @Date: 2025-05-29 13:48:20 
 * @Last Modified by:   carolsail 
 * @Last Modified time: 2025-05-29 13:48:20 
 */

import PoolManager from "./BJPoolManager";

export default class BJEffectManager {
    public static _instance: BJEffectManager = null

    public static get instance() {
        if (null == this._instance) {
            this._instance = new BJEffectManager();
        }
        return this._instance
    }

    play(effect: string, parent: cc.Node, options?: any, cb?: () => void) {
        const effNode = PoolManager.instance.getNode(`${effect}`, parent)
        if (options) {
            options.pos && effNode.setPosition(options.pos)
            options.label && (effNode.getComponent(cc.Label).string = options.label)
        }
        if (effNode.getComponent(cc.Animation)) {
            ...
            if (options) {
                // 缓动
                if (options.act) {
                    cc.tween(effNode).then(options.act).call(() => {
                        effNode.removeFromParent()
                        cb && cb()
                    }).start()
                }
